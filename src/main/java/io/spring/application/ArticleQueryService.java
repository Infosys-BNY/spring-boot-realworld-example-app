...
public class ArticleQueryService {
  ...
  public CursorPager<ArticleData> findRecentArticlesWithCursor(
    ...
    List<String> articleIds =
        articleReadService.findArticlesWithCursor(tag, author, favoritedBy, null, page);
  ...
  public ArticleDataList findRecentArticles(
    ...
    List<String> articleIds =
        articleReadService.queryArticles(tag, author, favoritedBy, null, page);
    int articleCount = articleReadService.countArticle(tag, author, favoritedBy, null);
    if (articleIds.size() == 0) {
  ...
  public ArticleDataList findBookmarkedArticles(String bookmarkedBy, Page page, User currentUser) {
    List<String> articleIds =
        articleReadService.queryArticles(null, null, null, bookmarkedBy, page);
    int articleCount = articleReadService.countArticle(null, null, null, bookmarkedBy);
    if (articleIds.size() == 0) {
      return new ArticleDataList(new ArrayList<>(), articleCount);
    } else {
      List<ArticleData> articles = articleReadService.findArticles(articleIds);
